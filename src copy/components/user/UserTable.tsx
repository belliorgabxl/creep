...
function Table({
  ...
  return (
    <div className="overflow-x-auto rounded-xl border border-gray-200 bg-white shadow-sm">
      {/* ตั้งขนาดฟอนต์มาตรฐานของตารางเป็น text-sm */}
...
function TH(
  ...
  return (
    <th
      scope="col"
      className={`px-4 py-3 text-sm font-semibold uppercase tracking-wide text-gray-700 ${alignCls} ${className}`}
      {...rest}
    >
      {children}
    </th>
...
function TD(
  ...
  const alignCls =
    align === "right" ? "text-right" : align === "center" ? "text-center" : "text-left";
  // เปลี่ยนเป็น text-sm เพื่อความสอดคล้อง
  return (
    <td className={`px-4 py-3 text-sm text-gray-800 ${alignCls} ${className}`} {...rest}>
      {children}
    </td>
  );
}
    ...
    <div>
      <Table>
        <THead>
          ...
          <TH className="w-2/14">ชื่อ</TH>
          <TH className="w-2/14">ตำแหน่ง</TH>
          <TH className="w-3/14">แผนก</TH>
          <TH className="w-2/14 text-center">สถานะใช้งาน</TH>
          <TH className="w-1/14" align="right">จัดการ</TH>
          <TH className="w-1/20" align="right"></TH>
        </THead>
