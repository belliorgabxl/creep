...
type QADetailModalProps = {
  qaId: string; // <-- this is the REAL id used to fetch details from API
  onClose: () => void;
...
type FormDataType = {
  id: string;      // API id
  code?: string;   // human-friendly code to show in UI
  name?: string;
  projects?: number;
  gaps?: boolean;
...
export default function QADetailModal({ qaId, onClose, onUpdate }: QADetailModalProps) {
  const [isEditing, setIsEditing] = useState(false);
  const [formData, setFormData] = useState<FormDataType>({
    id: qaId,
    code: qaId, // temporary until API returns a real code
    name: "",
    projects: 0,
    gaps: false,
  });
  ...
  // helper: robustly read project count from API record (handles various possible keys)
  ...
  // map API record -> FormDataType (using GetQaIndicatorsRespond fields plus tolerant reading for projects)
  ...
  useEffect(() => {
    // reset to minimal state while fetching
    setFormData({ id: qaId, code: qaId, name: "", projects: 0, gaps: false });
    setError(null);
    ...
    const fetchDetail = async () => {
      ...
      try {
        ...
        console.log("Fetched QA detail data:", data);
        if (!mounted) return;

        // API helper already returns array or object; normalize to single record
        ...
        // Map API response to formData and set it
        ...
        setError(msg);
        // keep existing minimal formData as fallback
      } finally {
          ...
          <div className="flex items-center gap-2">
            {!isEditing ? (
              <button
                ...
                className="inline-flex items-center gap-2 rounded-lg px-3 py-1.5 text-sm font-medium text-indigo-600 hover:bg-indigo-50 transition-colors"
              >
          ...
          <div className="rounded-xl bg-gradient-to-br from-indigo-50 to-indigo-100/50 p-6">
            <div className="flex items-start justify-between gap-4">
              <div className="flex-1">
                ...
                {/* title should show name (not code) */}
                {!isEditing ? (
                  ...
                  <input
                    ...
                    className="mt-3 w-full rounded-lg border border-indigo-200 px-3 py-2 text-lg font-semibold focus:border-indigo-500 focus:outline-none focus:ring-2 focus:ring-indigo-100"
                  />
                )}
              </div>
            ...
            <div className="rounded-xl border border-slate-200 bg-slate-50 p-4">
              ...
              {!isEditing ? (
                ...
                <input
                  ...
                  className="w-full rounded-lg border border-slate-300 px-3 py-2 text-xl font-bold focus:border-indigo-500 focus:outline-none"
                />
              )}
            </div>
            ...
            <div className="rounded-xl border border-slate-200 bg-slate-50 p-4">
              ...
              {!isEditing ? (
                ...
                <label className="flex items-center gap-3 mt-2 cursor-pointer">
                  <input
                    type="checkbox"
                    checked={!!formData.gaps}
                    onChange={(e) => setFormData({ ...formData, gaps: e.target.checked })}
                    className="h-5 w-5 rounded border-slate-300 text-indigo-600 focus:ring-2 focus:ring-indigo-100"
                  />
          ...
          <div className="rounded-xl border border-slate-200 bg-white p-5">
            ...
            <div className="space-y-3 text-sm">
              ...
              <div className="flex justify-between py-2">
                ...
                <span className={`font-medium ${formData.gaps ? "text-amber-600" : "text-emerald-600"}`}>
                  {formData.gaps ? "ต้องการการดำเนินการ" : "ปกติ"}
                </span>
              </div>
            </div>
          </div>
              ...
              <div className="flex gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-amber-800">
                    ตัวบ่งชี้นี้มีช่องว่างในการครอบคลุม ควรพิจารณาเพิ่มโครงการหรือปรับปรุงกระบวนการเพื่อให้ครอบคลุมมากขึ้น
                  </p>
                </div>
              </div>
            </div>
